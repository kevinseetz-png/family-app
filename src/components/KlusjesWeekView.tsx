    ...
    <div>
      ...
      <div className="space-y-3">
        {weekDates.map((date, i) => {
          ...
          return (
            <div key={dateStr} className={`rounded-lg border ${isToday ? "border-emerald-300 bg-emerald-50" : "border-gray-200 bg-white"} p-3`}>
              <div className="flex items-center gap-2 mb-2">
                <span className={`text-xs font-bold uppercase ${isToday ? "text-emerald-700" : "text-gray-500"}`}>
                  {DAY_NAMES[i]}
                </span>
                <span className={`text-sm ${isToday ? "text-emerald-700 font-medium" : "text-gray-600"}`}>
                  ...
                  {dayItems.map((item) => (
                    <li key={item.id} className="flex items-center gap-2 text-sm">
                      <button
                        ...
                        className="flex-shrink-0 h-5 w-5 flex items-center justify-center rounded-full border border-gray-300 text-xs"
      ...
      {undatedItems.length > 0 && (
        <div className="mt-4 rounded-lg border border-gray-200 bg-white p-3">
          <h3 className="text-sm font-medium text-gray-700 mb-2">Zonder datum</h3>
          <ul className="space-y-1">
