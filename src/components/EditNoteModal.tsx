...
export function EditNoteModal({ note, onSave, onClose }: EditNoteModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/50" onClick={onClose}>
      <div
        className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-800 rounded-t-xl shadow-xl p-6 max-h-[90vh] overflow-y-auto"
        role="dialog"
        aria-label="Notitie bewerken"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Notitie bewerken</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="edit-note-title" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Titel
            </label>
            <input
              id="edit-note-title"
          ...
          <div>
            <label htmlFor="edit-note-content" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Inhoud
            </label>
            <textarea
